...
/**
 * **The failed-pack banner is not here.** It used to be, set by
 * `LoadingScene.ts`'s `boot()` on the line before it showed the menu — and
 * `exit()` hides this whole scene, so the player saw it for as long as the
 * menu chunk took to arrive and no longer. It lives on the menu now
 * (`src/scenes/packBanner.ts`, `MenuScene.vue`), which is where spec §7 puts
 * it and the only screen that is still on the glass afterwards.
 */
