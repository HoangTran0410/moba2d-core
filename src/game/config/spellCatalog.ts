 ...
 * silence.
 *
 * `'reference:'`, not `'riot:'`, as of content-pack-and-repo-split batch 6
 * task 10: `packs/riot/` left this repository entirely, `installedPacks`
 * (the generated barrel `BUNDLED_PACK_ID` reads first) is empty with no
 * optional pack installed, and `BUNDLED_PACK_ID` falls through to the
 * reference pack — always here, per `install.ts`'s own doc comment.
 * Found by fix round 2 of that task's own report: with the literal still
 * `'riot:'`, every `qualifiedId` this build's registry actually produces
 * (`reference:Vera_Q`, ...) failed `bareCatalogId`'s `startsWith` check, so
 * `listSpellCatalog()` — the free-form kit builder's entire picker —
 * silently returned zero entries. `tests/game/config/spellCatalog.test.ts`'s
 * own pinning test would have caught this the moment it happened, except
 * that file is itself still dormant, importing `packs/riot/` directly — a
 * pre-existing, separate case of the same disease this task's whole fix
 * round is about, out of this round's own scope to also repair.
 */
const BUNDLED_PACK_PREFIX = 'reference:';
