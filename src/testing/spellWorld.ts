 ...
 * accepted.
 *
 * **This is the only honest way to drive a spell in a test.** Calling `onSpellCast()` by hand runs
 * one hook in isolation: no activation pattern, no recast budget, no `onComplete`, no resource
 * commit, no targeting rejection, no cooldown. Five assertions on one channelled ultimate were all
 * green against a spell that opened and shut its stage inside a single keypress, because every one
 * of them called the hook directly. `spell-runtime-drive-seam.test.ts` is the ban; this is the
 * thing that makes obeying it a one-liner.
 *
 ...
 *
 * Several abilities ship with `CAST_TIME_MS = 0` — an instant press is a deliberate feel choice,
 * and retuning it must not mean editing a test. But the runtime rules that only exist *while a
 * cast is in flight* — a UNIT target going invalid mid-cast, a resource committed at release
 * rather than at start — have no window to happen in at zero, so a test that drives them through
 * the shipped number stops covering anything the day someone zeroes it. That is exactly what
 * happened: four suites went red the moment five spells were made instant, and none of them was
 * actually asserting anything about the particular spells involved. The rule under test belongs to
 * the runtime.
 *
 *   const spell = new (withCastTime(SomeSpell, 250))(owner);
 */
