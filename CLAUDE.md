...
| `hud/config/PanelSection.vue` + `panelSections.ts` | the config panel's folding sections: header = fold handle + one-line summary of the values inside while folded; open state persisted as overrides over each section's default; `v-show` so e2e selectors still resolve — scripts call `openPanelSections(page)` after switching tabs |
...
| `config/championPool.ts` | which packs a random roll may land on, stored as the exceptions (`disabledPacks`); `poolOf` never returns empty; the one filter sits in `preset.ts`'s `randomChampionKit`, which every random anything goes through; switched from the die on each pack heading in `KitRoster.vue` |
| `config/matchHistory.ts` + `combat/MatchRecorder.ts` | the local match history: a 40-match ring written by `Game.matchId` (idempotent autosave every 30s of match time + on destroy/pagehide/hidden), evictions folded into a per-champion `archive`; mastery = archive + ring; `qualifies` gates junk restarts; off on LAN clients. Read by `KitRoster.vue` (badge) and `MatchTab.vue` (Trận gần đây) |
