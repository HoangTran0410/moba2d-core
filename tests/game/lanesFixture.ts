...
import { cachedLanesForTests } from '../../src/testing/lanes';

/**
 * Installs Summoner's Rift's real lane waypoints as the active match's lane
 * set — the same real, checked-in map `src/testing/setup.ts`'s
 * `installPackForTests` caches (via `src/testing/lanes.ts`) for every test
 * file's environment by default (see that function's own doc comment for
 ...
 * real `Game`).
 *
 * A describe block that installs its *own* lane set for one or two tests (a
 * laneless map, a synthetic one) and then wants later describes in the same
 * file to keep seeing the ordinary ambient default calls this in its own
 * `afterEach`, right after `resetLanesForTests()` — `resetLanesForTests()`
...
export function installSummonersRiftLanesForTests(): void {
  setActiveLanes(cachedLanesForTests());
}
