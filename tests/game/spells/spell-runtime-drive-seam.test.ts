 ...
 *
 * "Every spell test" spanned two directories from batch 4 task 5, when the
 * champion-named tests moved to `tests/packs/riot/spells/`, through
 * content-pack-extraction batch 6 task 6, which moved that whole directory
 * again — this time into `packs/riot/tests/`, the pack's own tree, gated by
 * the pack's own `check-seams` (`src/seams/spellRuntimeDrive.ts`'s
 * `checkSpellRuntimeDrive`, the engine-side rule this file is the worked
 * example for) rather than by anything under `tests/`. This file's own
 * population is back to one directory — the seam scans and generic
 * behaviour tests here, in core's own tree, which is the half of "every
 * spell test" no pack's own gate can reach.
 */
const SPELL_TESTS_DIRS = [__dirname];
 ...
 * they were written.
 *
 * Down from 32 to 3 when content-pack-extraction batch 6 task 6 moved the
 * champion-named tests out of this scan's population entirely (see
 * `SPELL_TESTS_DIRS`'s own comment) — not because 29 files were migrated,
 * but because they are now the pack's own debt, declared in
 * `packs/riot/tests/seam-debt.mjs`'s `grandfatheredTests` and checked by
 * the pack's own `check-seams`, same rule, different gate.
 */
...
describe('a spell test presses the spell, it does not call its hooks', () => {
  ...
  it('the debt only shrinks', () => {
    expect(GRANDFATHERED.size).toBeLessThanOrEqual(3);
  });
});
