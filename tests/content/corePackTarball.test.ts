 ...
 * ## `.tmpl` is text too
 *
 * Content-pack-and-repo-split batch 6 task 8's own fix round: `files`
 * started shipping `scripts/templates/pack/**` (`moba2d-pack-new`/
 * `moba2d-pack-add`'s eleven `.tmpl` files) the moment that task landed,
 * and `TEXT` below did not know the extension existed — a template author
 * reads and copies its prose the same way anyone reads a `.ts` file, and a
 * champion, monster or spell-id example written into one for illustration
 * is exactly as public as the same string in `src/`, the moment `files`
 * carries it. Property 1 above is stated as "every text file in the
 * tarball"; a filter that silently excludes a whole shipped extension
 * is not that, regardless of what it happens to catch today. No leak was
 * found when this was fixed — every template already used invented,
 * non-Riot names (`Hero`, `Q_DAMAGE`, `Bolt`) — but the property this test
 * exists to hold was false for eleven files for as long as they shipped
 * unscanned, and the next `.tmpl` (or the next new shipped extension
 * entirely) needs this list to already be honest about what "every text
 * file" means.
 */
describe("@moba2d/core's published tarball", () => {
  const TEXT = /\.(ts|tsx|vue|mjs|cjs|js|json|css|html|svg|md|txt|yml|yaml|tmpl)$/;
