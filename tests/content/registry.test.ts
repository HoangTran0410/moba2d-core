 ...
 * the riot pack ships 60-odd champions, the reference pack ships exactly one,
 * Vera. The point of each assertion is the `toHaveLength(first)` / `spellIds()`
 * line after it, and a floor of 0 — which this used to be, for the
 * not-installed branch — would let those pass against an empty roster: `0` is
 * not the reference pack's real minimum, `1` is, and only `toBeGreaterThanOrEqual`
 * against the true floor (rather than `toBeGreaterThan` against one less than
 * it) says that honestly. Whole-branch fix pass: both assertions below moved
 * to `toBeGreaterThanOrEqual` to match.
 */
const ROSTER_FLOOR = packIsInstalled('riot') ? 30 : 1;
...
describe('the content registry', () => {
  ...
  it('installs once, not once per read', () => {
    ...
    expect(first).toBeGreaterThanOrEqual(ROSTER_FLOOR);
    contentRegistry();
    contentRegistry();
  ...
  it('contentCatalog() alone installs the data half and nothing more', () => {
    ...
    expect(registry.champions().length).toBeGreaterThanOrEqual(ROSTER_FLOOR);
