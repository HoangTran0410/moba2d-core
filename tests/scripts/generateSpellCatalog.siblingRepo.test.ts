...
describe('generate-spell-catalog against a pack outside this repository', () => {
  async function buildSibling(): Promise<string> {
    ...
    // The pack's own tree — the four things this generator reads. Batch 6
    // task 9 added `catalog.config.mjs`: the tree's own shape (barrels,
    // `isPackFactory`, `packId`, `assetManifestOutputPath`) used to be a
    // `PACK_SPELL_TREES.riot` entry inside core itself, and now ships with
    // the pack the same way `spells/`, `vfx/` and the asset manifest do —
    // so a sibling checkout of the pack has to carry it too, or this
    // simulation is testing a pack shape that no longer exists.
    ...
    await cp(join(repoRoot, 'packs/riot/catalog.config.mjs'), join(root, 'catalog.config.mjs'));
