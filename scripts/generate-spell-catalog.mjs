 ...
 * `"packs/riot"` as a location anywhere.
 *
 ...
 *
 * ## Fix round 2: the tree's *shape* is a caller argument too, not a table
 *
 * Content-pack-extraction batch 6 task 9: round 1 stopped this file from
 * hardcoding a pack's *paths*, but `PACK_SPELL_TREES` still hardcoded the
 * pack's *name* and its tree's shape (barrels, `isPackFactory`, `packId`,
 * `assetManifestOutputPath`) — a table in core's own tooling holding a
 * pack's own knowledge about its own layout, and a script that knows a
 * pack's name is a script that breaks, or silently does nothing, the day
 * that pack is gone. `PACK_SPELL_TREES` is gone; a named tree (`--tree=
 * <name> --root=<path>`) now reads its definition from `<root>/catalog.
 * config.mjs` and imports its default export as the tree object — the
 * pack's own file, stating the pack's own shape, the same relationship
 * `<root>/seam-debt.mjs` already has with `check-seams.mjs`. `--tree=` is
 * kept as the argument the pack's `package.json` already passes; nothing
 * in this file looks it up any more; a `--tree=` with no matching
 * `catalog.config.mjs` at `--root=` is an error naming the missing file,
 * not an "unknown tree" — there is no registry left to be unknown against.
 * `packs/riot/catalog.config.mjs` is that file for this repository's own
 * pack today, holding exactly what `PACK_SPELL_TREES.riot` held.
 *
 * ## Why this exists
 *
...
export const CORE_SPELL_TREE = {
  ...
  // Batch 4 Task 3 moved `src/game/gameObject/spells/` out into a pack's own
  // tree (see this file's header, "Fix round 2" — a pack now states its own
  // tree in its own `catalog.config.mjs`, not a table here) — `BasicAttack`
  // is now the only spell core generates a catalogue entry for at all, which
  // is the whole point of `coreSpells/`'s own header: a mechanic every pack
  // presupposes, not content.
  barrels: [
...
/** The pack-side file a named tree's own shape now lives in — see this file's header, "Fix round 2". */
const CATALOG_CONFIG_FILENAME = 'catalog.config.mjs';
...
if (invokedDirectly()) {
  ...
  if (treeName && !rootValue) {
    ...
    if (!treeName) {
      generateSpellCatalog(process.argv.includes('--check'), CORE_SPELL_TREE, treeRoot).catch(
        error => {
      ...
      // A named tree states its own shape now, beside the tree it
      // describes — no table in this file to look `treeName` up in any
      // more, so there is nothing left to be "unknown" against. Only
      // whether the file it names is actually there.
      const configPath = resolve(treeRoot, CATALOG_CONFIG_FILENAME);
      if (!existsSync(configPath)) {
        console.error(
          `--tree=${treeName}: no ${configPath} — a named tree default-exports its own shape ` +
            `from a ${CATALOG_CONFIG_FILENAME} beside it now; see this file's own header, ` +
            '"Fix round 2", for the shape that file has to export.'
        );
        process.exitCode = 1;
      } else {
        const { default: tree } = await import(pathToFileURL(configPath).href);
