...
import { installedContentPackages } from './installed-packs.mjs';
...
// would notice.
//
// Gated on whichever content packs are actually installed — the same
// distinction `tests/support/installedPacks.ts` draws for the source scans.
// Every `spell-<champion>-*.js` chunk comes from an installed pack's own
// spells (`vite.config.ts`'s rule matches that path and no other); the
// reference pack's four spells ride in `pregame` by design and publish no
// `generated/spellModules.ts` at all, which is what drops it out of
// `spellPacks` below on its own — no pack name is excluded by hand. So in a
// checkout with no optional pack installed the honest count is zero and a
// floor would fail a perfectly good build — which is exactly what
// `npm run verify:without-packs` measured the first time it got this far.
//
...
// would have stayed green through a rule that lost a third of the split. Each
// installed pack's own generated module map is exactly what Rollup splits, and
...
// each pack's resolved directory, which is the workspace symlink today and a
// real `node_modules` directory once a pack is a repository of its own.
const spellChunks = readdirSync(assets).filter(name => /^spell-.+\.js$/.test(name));
const spellPacks = installedContentPackages(root).filter(pack =>
  existsSync(join(pack.dir, 'generated/spellModules.ts'))
);
for (const pack of spellPacks) {
  ...
  if (missing.length > 0) {
    failures.push(
      `${pack.name}: ${missing.length} of ${expected.size} per-champion spell chunks are ` +
