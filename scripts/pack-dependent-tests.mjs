 ...
 *
 * Batch 6 task 6 is that day, for the 69 (`tests/packs/riot/**`, the
 * "living under `tests/packs/<pack>/`" signal below) plus the one this
 * file's own `walk` counted alongside them (Task 5's `noCoreReach.test.ts`)
 * — moved into `packs/riot/tests/`, which is not under `tests/` at all, so
 * that signal now matches nothing and this function's returned list is the
 * *other* half of the 138 (69 files, unchanged), never the union. Read as
 * "most of the population this file exists for goes away", not "this file
 * goes away": signals 1 and 3 below are exactly what is left, and both are
 * still real — `tests/game/spell/registry.ts` and the 68 files behind it
 * did not move, because they are core mechanism tests reading a real pack
 * spell to check an engine rule, not the pack's own spell tests.
 *
 * ## Why a closure and not a glob
 *
 * `tests/packs/riot/**` used to be 69 of the 138 and would have been a
 * one-line glob (moot since batch 6 task 6: that directory is gone, moved
 * whole into the pack). The other 69 are spread across `tests/game/` (51),
 * `tests/e2e/` (10), `tests/content/` (5), `tests/scenes/`, `tests/scripts/`
 * and `tests/wiki/` (1 each) — they import a real
 ...
 * `Failed to load url ../../packs/riot/scripts/generate-assets.mjs`.
 * Content-pack-and-repo-split batch 6 task 10 moved both the test and the
 * script it drove out of this repository entirely (the rest of
 * `scripts/wiki/` with it), so this signal has no member today — kept wired
 * rather than removed, since a future test reaching a future pack through a
 * build script is exactly the shape this closure exists to catch, and the
 * closure spans `tests/` and `scripts/` both, returning only the `tests/`
 * half.
 *
 ...
 *
 * Three shapes, all found by running the drill (or, for the third, the
 * whole-branch fix pass) and reading the failure:
 *
 ...
 *   - **the pack's *names*, hardcoded as plain string literals.** Eight
 *     `tests/e2e/` drivers, none of them caught by the import-graph scan
 *     above because none of them imports a pack path — they name riot
 *     content in data: a `championName: 'Nasus'` in a seeded match config, a
 *     literal `'riot:summoners-rift'` map id, an assertion of `mapSize 6400`
 *     / `395 polygons` / `22 turrets`, a `.kit-shelf[data-champion="Zed"]`
 *     selector. Whole-branch review of this batch found them: the earlier
 *     "four e2e scripts the departure broke" (see this batch's own handover,
 *     §7) were the subset the import scan *could* see; these are the rest.
 *     Worse than a hard failure for the ones naming a `championName`:
 *     `src/game/preset.ts` resolves an unknown name to a *random* installed
 *     champion rather than throwing, so each ran to completion against
 *     whichever reference-pack champion the fallback picked and reported
 *     pass/fail about abilities it never loaded. Not repointed at the
 *     reference pack here — each genuinely needs riot's specific roster size,
 *     map geometry or named kit (Cho'Gath's stack mechanic has no
 *     reference-pack analogue at all) — so they are recorded pack-dependent
 *     instead, the same as the two shapes above, rather than left silently
 *     green against a random champion or silently absent from every gate.
 *
...
const PACK_CONTENT_FIXTURE_TESTS = {
  riot: [
    ...
    // The eight string-keyed e2e drivers — see the third shape above.
    'tests/e2e/verify-map-picker.mjs',
    'tests/e2e/drive-execute-taunt-terrain.mjs',
    'tests/e2e/drive-practice-panel.mjs',
    'tests/e2e/drive-match-config.mjs',
    'tests/e2e/drive-roster-stats.mjs',
    'tests/e2e/drive-hold-move-buff.mjs',
    'tests/e2e/drive-kit-builder.mjs',
    'tests/e2e/measure-chogath-stacks.mjs',
  ],
};
 ...
 * `Vision`) and between them it is around a hundred tests of stat ceilings,
 * regen, vision, line of sight and blackboard bucketing — core mechanism,
 * every one of them — of which round 1 skipped nine `it()`s, not one apiece:
 * `PregameConfig.test.ts` 2, `Stats.test.ts` 1, `TeamBlackboard.test.ts` 1,
 * `Vision.test.ts` 5 (`it.skipIf(!packIsInstalled('riot') || ...)`, counted
 * directly against each file's own `it.skipIf` calls — whole-branch fix pass
 * correction; an earlier draft of this comment said "a single `it()` apiece",
 * which was true of three of the four files and not of `Vision.test.ts`).
 */
 ...
 *      the pack whose tests it holds, so it needs no list of its own.
 *      Content-pack-extraction batch 6 task 6 moved the one pack that had
 *      such a directory (`tests/packs/riot/`) into `packs/riot/tests/` —
 *      not under `tests/` — so this signal now matches nothing in an
 *      ordinary checkout. Kept rather than deleted: the next pack extracted
 *      the same way grows a `tests/packs/<name>/` directory of its own
 *      before it, too, moves into its pack, and this signal is what covers
 *      that pack's tests (the ones that reach their subject only by
 *      `readFileSync`, invisible to signal 1) for exactly as long as they
 *      live here;
