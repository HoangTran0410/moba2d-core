...
Deferred, plumbing ready: 30s out-of-combat auto-checkpoint cadence (`auto` flag + "Tự động" tag exist); turret destroyed-state (a rewind keeps turrets as they stand); toggle/channel spells restore not-sustaining. Known edge: rewind after the bag CHANGED since capture → a rebuilt item passive re-arms fresh next frame and can replace that one restored passive-buff's state (identical-bag rewinds keep full fidelity).
