...
A content pack (`packs/reference/`, and any pack installed alongside it) may
not import `Spell`/`SpellObject`/a buff/anything else out of core as a
*value* — `tests/content/packBoundary.test.ts` fails the build over it, and
`moba2d-check-seams`'s `pack-core-boundary` rule fails a pack's own build the
same way outside this repository. Every spell file's default export is
instead a function that **receives** `api: ContentApi` and returns the class:
...
`AssetManager.get(key)`, and so on — see `packs/reference/spells/Vera_Q.ts`
for the full surface, worked. (Core-internal code — anything under `src/` —
reaches the same classes as plain value imports through the `@/` alias
instead; `api` exists for content, which is never allowed that alias.)
...
```sh
npx moba2d-pack-add spell Bolt --champion Vera --slot W
```

Run from inside a pack (found by walking up from the current directory, never
by a hardcoded path — this works the same from a pack's own root or a nested
directory inside one). It writes the spell and its test — already driven
through `press()`, never a lifecycle hook — and registers the import, the
champion's kit slot, and the code-half factory in `pack.ts`. It renders a
fixed starting shape (a straight `DIRECTION` bolt, one hit, the memoized
factory pattern from above already applied): use it even when the ability you
are building is not a bolt, then rewrite the body — `activation`, `targeting`,
and everything below `castSpec` — to describe the real ability. `champion`,
`map` and `monster` are named in the same command but refuse to run; add those
to `pack.ts` by hand, using what is already there as the model.
...
implementation will settle it. This engine has had abilities rebuilt from
scratch for exactly this reason: the code was correct and the ability was
wrong, because nobody had written down what the player was supposed to see
before writing what the computer was supposed to do.
...
| A `UNIT` spell declares `targetTeam` + `targetingRequest` + `press()` | `unit-target-team` seam (`npm run check-seams`) |
| A spell that picks a unit filters on `visibleTo`, and never reads `visibleToPlayerTeam` | `target-vision` seam (`npm run check-seams`) |
| A dash hooks `onDashUpdate`, never `onUpdate` | `dash-onupdate-seam.test.ts` |
| An effect painting past its centre has `getDisplayBoundingBox` | `spell-object-display-box` seam (`npm run check-seams`) |
| Ground art names its layer, never a magic z-index | `ground-decal-zindex.test.ts` |
| Mana moves only through `effectiveMana`/`spendMana` | `mana-spend-seam.test.ts` |
| A legacy spell declares `targetingMode` | `targeting-mode-declared` seam (`npm run check-seams`) |
| A pack reaches core only through `api`, never a value import | `pack-core-boundary` seam (`npm run check-seams`) |
| A pack resolves art through its own manifest, never a bare core key | `pack-asset-key` seam (`npm run check-seams`) |

Two rules used to have their own dedicated `.test.ts` files (`unit-target-team-seam.test.ts`, `target-vision-seam.test.ts`) and are folded into `check-seams` now — the table above names where each rule actually lives today, not its history. **The Vietnamese display-name rule that used to sit here — every spell's name is the string the official client ships — is gone from this table on purpose**: it was checked against Data Dragon's `vi_VN` locale, which only has an answer for a Riot champion's real name, so it is specific to that content and lives in the pack repository that content moved to, not in this generic mechanism doc.
...
therefore disagrees. An ability that reads current stats into its `castSpec`
freezes whatever those stats were on the very first cast for the rest of the
match. To vary a cooldown, write `this.currentCooldown = this.reducedCooldown(n)`;
to vary anything else, put it in a hook.

**`RECAST` gives you exactly one recast unless you ask for more.**
`SpellRuntime.recast()` completes the activation after the first one. An
ability whose whole shape is "press again N times" — a channel that fires
several shots, one per press — needs `active: { recasts: N }`, and
`recastDelayMs` is then the gap between *consecutive* recasts. `onRecast` is
also handed the context of the **opening** press, so aim each repeat with
`this.aimPoint`, never the context argument.

## 1. Choose activation and targeting

Override `castSpec` with one activation gesture and one targeting mode:

- `PRESS`: one press — instant, or after a fixed windup.
- `HOLD_RELEASE`: press, charge, then physical release — a shot that flies further the longer you hold it.
- `RECAST`: press again while `ACTIVE` — a channel that keeps going, or an ability that fires again on a second press.
- `TOGGLE`: press again to end an `ACTIVE` effect — a stance you turn off yourself.
- `TAP_OR_HOLD`: release behavior depends on the charge duration — a tap does one thing, a hold does another.
...
**Auto-locking spells.** A legacy spell can pick its own target from inside
`onSpellCast`/`checkCastCondition` — nearest enemy or ally within a fixed
radius of the caster, occasionally broken among several candidates by
whichever is closest to the cursor — and never read `context.target`. That
reads like `UNIT`, but declaring it `UNIT` hands the *cast itself* to
`TargetResolver`, which (unlike a spell's own hand-rolled lookup) requires the
cursor to be sitting almost on top of a body before it will resolve at all —
see `TargetResolver.resolve`'s `UNIT` branch. A spell shaped like this needs
the `targetingRequest` + `press()` override a real `UNIT` spell supplies
(below); without it, `UNIT` would silently swallow the key press whenever the
mouse is not precisely on a target. **Every `UNIT` spell must explicitly set
`targetTeam: 'ENEMY'`** (or `'ALLY'`) in its `targetingRequest` and guard
against self/friendly target in `onSpellCast`/`checkCastCondition`; omitting
`targetTeam` defaults to `'ANY'`, allowing the caster to self-target and deal
damage to themselves when no enemy is around — four shipped kits in this
engine's history have done exactly that, each the day they were written. An
auto-locking spell that has not yet been given the `targetingRequest`/`press()`
treatment is declared `targetingMode = 'SELF'` instead: it is the only mode
that changes nothing about when the cast is allowed to start, on touch or on
desktop. Turning one into a true `UNIT` spell — so a drag can deliberately
choose among several candidates and the touch layer can highlight who is
about to get hit — is a real improvement, but it is a target-acquisition
feature, not a targeting-mode correction; give it the `targetingRequest`/
`press()` treatment and its own test when you do. `Game.createSpellContext`
(`src/game/Game.ts`) is where the ordinary key-press path already runs
`TargetResolver.resolve(spell.castSpec.targeting, { ...spell.targetingRequest, ... })`
before `press()` ever sees the context — read it alongside
`TargetResolver.ts`'s own `UNIT` branch before writing a `press()` override,
rather than guessing the shape from this description.
...
without it: an ability that finds "the nearest enemy within radius R" by
querying `objectManager` directly found a jungle monster through a solid wall,
on a screen showing nothing but fog, because the query never asked whether the
caster could see what it picked. The filter asks
...
everyone it overlaps — a ground-targeted pulse ticking on the champion hiding
in the bush it landed on is correct, and adding the filter there would be the
bug. The question to ask of a query is whether picking a unit out of it means
the caster *chose* that unit.
...
filtering happen. `tests/e2e/smoke-new-champions.mjs` (`npm run e2e:champions`)
drives the whole installed roster through it in a real browser and checks
each cast was accepted; grow its `ROSTER` constant the day a second
reference-pack champion exists.
...
What picks each form:

- **`HELD`** — everything with a cast time or a channel: an instant press, a
  windup, a channeled ultimate. Omitting `interrupts` means this, so it is the
  right default for most spells.
- **`AIMED`** — a charged shot the champion is physically drawing while free
  to strafe. Walking survives; crowd control still ends it, because the
  champion's own hands are still what is holding it.
- **`TETHERED`** — an effect that stands in the world but stays leashed to its
  caster and billed to their mana, so they may walk and be knocked about
  without ending it, and only losing control of themselves still does.
- **`INDEPENDENT`** — an effect already out of the caster's hands and running
  on its own clock: summoned and growing toward a fixed detonation, already
  called down, or already carrying its own momentum. Nothing short of the
  caster's death reaches it once it is loose.
...
- **`suspendedBy: [Stasis]`** — a buff that *pauses* the watcher rather than ending what it guards. A stasis effect reads as a stun and a silence at once, but whatever the champion was sustaining is still theirs when it ends; add a buff here rather than reaching for a wholesale opt-out.
...
- **A standing attack order owns the caster's movement.** `BasicAttackController` writes `owner.destination` every frame it has a target — walking into range, then `stopMovement()` once there. A spell whose effect *is* movement (a rolling dash that keeps going until it hits a wall or a body) is not cancelled by that, but it is held still by it, which looks the same.
- **A buff's own end conditions.** A buff ends on its duration or when something calls `deactivateBuff()`. If a spell's live phase is really the buff's lifetime, give the spell an `active` window of the same length and close it from the effect, so the runtime state and the thing the player can see agree.
...
## 4. Bind presentation to lifecycle
...
without ever missing the cache.
...
cable) rather than only around yourself. Half a dozen effects have shipped
without it in this engine's history, each the same way: the effect worked,
and simply stopped being drawn the moment its centre point left the camera.
`tests/game/spells/aoe-display-bounds.test.ts` and the `spell-object-display-box`
seam (`npm run check-seams`) are what catch it.
...
It reads exactly like a callback, which is why three separate dash abilities
shipped with it in this engine's history and none was caught at first: each
still dealt its damage — to whatever happened to be standing next to the
caster, since the champion never actually moved — and still ended on time, so
the only thing wrong was invisible from the file itself.
...
rejection. A recast ultimate's five assertions were once green against a build
that raised and dropped its whole activation inside a single keypress — every
recast but the first was untested, because the hook it called never went
through the recast budget at all. `spell-runtime-drive-seam.test.ts` bans it;
several files predate the ban and are listed there as debt.

The same shift is what surfaces a spell's real shape: migrating one legacy
test revealed a `castTimeMs` windup nobody had been testing, and another
revealed that an interrupted channel still landed one more volley than the
interrupt should have allowed.
...
`verify` checks the generated asset manifest and spell catalogue, both TypeScript boundaries, every Vitest test, the production build, the per-champion chunk shape, and the source-scan seams.

## 6a. Look at it, once, in the real renderer

`verify` cannot see whether an effect is legible, and no unit test ever will —
so a new spell is not done until somebody has looked at it running.
`tests/e2e/shoot-new-champion-vfx.mjs` (`npm run e2e:vfx`, or run it
directly) shoots the installed roster's own abilities — add an entry for
...
Its own `ALL_CASTS` list is only the reference pack's four abilities today;
add an entry the same shape the moment a second champion exists to shoot.
...
2. Does anything the player must *find* — a ground trap, a dropped item — read
   at a glance? A 26px pale-grey shape on a pale floor with no outline does not.
   Anything on the floor wants ~40px of longest dimension and a
...
## 7. Measure a caster-centred range through `Reach`
...
apart, and a size-buffing ultimate can grow a champion to `MAX_UNIT_SIZE` —
radius 82.5 — so an enemy's centre can never come nearer than 110. A range
shorter than that stops working outright, not partially.
...
An ability that triggers off basic attacks (an on-hit poison stack, lifesteal, an attack-speed stack) subscribes to the event, it does not reimplement the swing. Both events come from `src/game/combat/`:

- `EventType.ON_ATTACK` fires when a swing starts. Payload is the attacking unit, nothing else. Use it to react to the commitment — a channel that breaks the moment its owner attacks reads this.
...
(`BasicAttackHit` is a type, imported from `@moba2d/core/content/types` like `CastContext`.)
